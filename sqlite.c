...
,mm->msgtype, a->flight, a->flight, mm->addr, mm->altitude, mm->vert_rate, a->lat, a->lon, a->speed, a->track, a->messages);
  ...
  sql = sqlite3_mprintf( "INSERT OR IGNORE INTO trackslog (modes, alt, vr, lat, lon, speed, heading, flight, last_update) VALUES ('%06X', '%d', '%d', '%1.5f', '%1.5f', '%d', '%d', '%s',CURRENT_TIMESTAMP);" , \
			 a->addr, a->altitude, a->vert_rate, a->lat, a->lon, a->speed, a->track, a->flight);
  rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
  if( rc != SQLITE_OK ){
   fprintf(stderr, "SQL error: %s\n", zErrMsg);
      sqlite3_free(zErrMsg);
